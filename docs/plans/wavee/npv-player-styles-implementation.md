...
sealed class DeckClock : Component
{
    ...
    const float TickMs = 1000f / 30f;      // = AmbientPowerPolicy foreground ambient cadence (see AmbientPowerPolicy.PluggedLoopHz — the plugged-in host.Animation.DefaultLoopHz)
...
## Port to `release/0.2.9-perf` — 2026-09-10

The feature was first built on `feat/catalog-state` (an archive branch) and ported here. The 0.2.9 bridge publishes no
transport phase, no separate play intent and no seek/scrub signals, so three things differ from the text above:
`PlaybackPhase` is declared in `Deck/Model/DeckInput.cs` and DERIVED by `DeckClock.PhaseOf` (error → Failed,
buffering → Buffering, playing → Playing, not playing within 1.5 s of the end → Ended, else Paused); `PlayWhenReady`
is `IsPlaying`; `PlaybackBridge` gains two UI-side signals, `SeekTargetMs` and `ScrubTargetMs`, written only by
`DeckGesture` and cleared by `DeckClock` once the reported position converges (or after 2 s). There is no audible
preview while dragging a grip: the medium moves with the pointer and `CommitSeek` fires once on release, matching the
seek bar on this branch. The CHANGELOG intro now reads "one new feature" instead of "no new features".
