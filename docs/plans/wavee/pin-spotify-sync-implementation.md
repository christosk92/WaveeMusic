...
**§5 (Liked Songs wire uri) status: RESOLVED 2026-09-11.** A live `--spotify-collection pins` run (account
`31u***tq`) returned the full `ylpin` set: `spotify:album:…`, three `spotify:artist:…`, **`spotify:collection`**
(Liked Songs — the bare spelling, no username), **`spotify:folder:36405e1711f88d9c`** (a rootlist folder, keyed by
the same 16-hex client-minted group id the rootlist carries as `spotify:start-group:<hex>:<name>`) and
`spotify:playlist:37i9dQZF1EP6YuccBxUcC1` (an editorial daylist, not in the rootlist). `PinSyncRules.LikedWireUri`
is now `spotify:collection`, folder pins sync two-way (`spotify:folder:<hex>` ↔ `folder:<hex>`), and everything the
app cannot represent (Your Episodes, Local Files, audiobooks, tracks/episodes/prereleases, unknown schemes) is dropped
before the store and never written back — it survives on the server untouched. A pinned item is also removed from
every ordinary list/tree/shortcut row of a document that has a Pinned section (`SidebarRowPlanner.HiddenByPin`), so it
never renders twice.
...
**The Liked Songs pin — captured.** The `/json` push describes it as `{"type":"collection"}` with no identifier;
the `/paging` response (captured 2026-09-11) carries it as the bare **`spotify:collection`**. `PinSyncRules` writes
exactly that (`LikedWireUri`) and accepts every spelling `EntityUri.IsLikedCollection` recognises on the read side
(`spotify:collection`, `spotify:collection:tracks`, `spotify:user:<u>:collection`, `spotify:user:<u>:collection:tracks`).

**Every uri shape the set can carry, and what the app does with it:**

| Wire uri | Meaning | Handling |
|---|---|---|
| `spotify:playlist:X` (own / followed / editorial) | playlist | syncs; an unlisted one hydrates via `HydratePinAsync` |
| `spotify:album:X`, `spotify:artist:X`, `spotify:show:X` (audiobooks ride `show`) | library entity | syncs |
| `spotify:collection` | Liked Songs | syncs; written as exactly this |
| `spotify:collection:tracks`, `spotify:user:U:collection[:tracks]` | legacy Liked spellings | read side collapses to `liked`; never written |
| `spotify:folder:HEX` (16 hex = rootlist group id) | folder | syncs ↔ `folder:HEX`; a folder absent from the rootlist renders disabled with a reason |
| `spotify:collection:your-episodes`, `spotify:user:U:collection:your-episodes` | Your Episodes | not represented (no route); preserved on the server |
| `spotify:local-files` | Local Files | route retired; preserved |
| `spotify:audiobook:X`, `spotify:book:X`, `spotify:track:X`, `spotify:episode:X`, `spotify:prerelease:X`, `spotify:station:…`, unknown | — | not represented; preserved |

The preservation invariant holds structurally: the only writer of `set="ylpin"` is `SetPinnedAsync`, reached solely
from `SidebarPinSync.OnLocalPinChanged`/`MigrateLocalPins`, both gated on `PinSyncRules.TryWireUri != null`;
`CollectionSets.AcceptsUri` filters adoption and `TryPinId` is the second fence.
`SidebarPinSyncTests.UnrepresentableRemotePins_ProduceNoLocalPinAndNoWrite_EvenAfterMigrationAndConvergence` pins it.
...
| `liked` (route pin, `SidebarPinId.LikedSongsUri`) | AppRoute | yes | `spotify:collection` (captured, §0.1) |
| `pl:wavee:playlist:X` | Playlist (session-local provider) | **no** | — |
| `folder:<groupId>` | Folder | yes | `spotify:folder:<groupId>` (the rootlist's 16-hex client-minted id) |
...
## 5. First task before any code: confirm the Liked Songs wire uri — DONE (2026-09-11)

Captured with `--spotify-collection pins` (run from a real terminal with `Start-Process … -Wait -NoNewWindow`; see
`.claude/skills/wavee/probes.md`): the wire spelling is `spotify:collection`. `PinSyncRules.LikedWireUri` carries it.
