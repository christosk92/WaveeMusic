...
`Wavee.UI.WinUI` publishes as **Native AOT** for Release configurations (both packaged MSIX and sideload `Release-Unpackaged`): `PublishAot=true`, `SelfContained=true`, `WindowsAppSDKSelfContained=true`, `IlcOptimizationPreference=Size`. Debug stays JIT so XAML Hot Reload, EnC, and fast F5 cycles work normally — the analyzers run in Debug too, so reflection regressions are caught at compile time without needing a publish cycle. The Release MSIX has no .NET runtime or WinAppSDK framework dependency; AOT cuts the native portion roughly in half versus framework-dependent SelfContained.
