...
### `SidebarRowSpec.Style` defaults to `Cluster` — a spec built without a pane config renders the landed anatomy

`SidebarRowSpec.Style` is stamped from `SidebarPaneConfig.RowStyle` only inside `SidebarPaneSlot`'s builders — a
row built anywhere else (the rail folder flyout, `LibraryV3NavBand`'s hand-placed entity tiles) has no config to
read and simply gets the struct default, `SidebarRowStyle.Cluster`. That is deliberate, not a gap: those rows want
today's landed anatomy (trailing cluster, pin glyph in the cluster) regardless of which mode is active, and only
rows built by `SidebarPaneSlot` for a `RowStyle.Slot` config (Library V3's own list) should ever see the one
trailing slot / pin-mark-in-subtitle behaviour. Never branch on `Config.Design` to decide this — branch on the
spec's own `Style`.
...
| a private-use glyph codepoint MUST be a `\uXXXX` escape, never a literal character | A pasted private-use character is invisible in most editors/terminals, so a diff or grep pass can carry a mangled one through unnoticed (UTF-8 bytes mis-decoded as Windows-1252 is the classic failure). The durable fix is always the literal escape text `\uXXXX`, never a pasted glyph, with the font named explicitly (`Icon`'s `family` param — reading only the codepoint against the wrong face is the OTHER way this renders as tofu, e.g. `WaveeIcons` codepoints resolved against Segoe Fluent). `Modes/LibraryV3/LibraryV3Header.cs`'s title carries no leading glyph any more — V3.1's chrome sweep deleted it (the title is text-only, per the header's width budget) — so this rule has no live V3 example today; it still binds the next glyph literal anyone pastes in. |
...
### Slot rows reserve `TrailingLaneWidth` (42) on the right — the "…" is a flex sibling, not an overlay

A `RowStyle.Slot` row's trailing slot (`SidebarRowGeometry.TrailingSlotWidth` 28, inside `TrailingLaneWidth` 42 =
`LeadingGap` + `TrailingSlotWidth` + `RowInsetRight`) is a real flex child laid out beside the title, not a
`ZStack` overlay painted on top of it — that is exactly what the Cluster anatomy did, and exactly what Slot deletes
on this path. Never add a second trailing reserve (a second `Width`/`Padding` carve-out, or a `ZStack` overflow
button) alongside it: the row's own `Padding` right already accounts for `RowInsetRight`, so a second reserve
double-charges the row's content width and either clips the title early or reopens the two-widths-of-dead-space
bug the single slot was built to close.
...
| `Shared/SidebarNavBandModel.cs` | **NOT a deletion candidate any more.** Its production caller is the nav band's own hand-placed entity tiles: `Modes/LibraryV3/LibraryV3NavBand.cs` calls `RouteKeyOf`/`SelectsRoute` for the shortcut band's rows (Library V3's nav band left the document as a section entirely and became fixed chrome — see the `SidebarPaneConfig.RailHead` entry below). `KindOf`/`Shape` still have none. `SidebarNavBandTests` drives the model directly either way. Its own file header still describes a `SidebarNavBand` *component* that no longer exists — that part is accurate. As of V3.1 the band carries no destination tiles at all (Albums/Artists/Podcasts/Local files never lived here; Liked Songs is the document's own `v3.system` row) — it is exactly the shortcut band's rows, nothing more. |
| `SidebarPaneConfig.RailHead` | **NOT deleted — re-added (W3), for exactly one mode.** Phase 1 had removed it on the theory that the Shortcuts band would always be an ordinary document section; that stays true for Classic/Curated, which still pass `RailHead = null` and get their rail tiles from `ShowInRail`. Library V3 is the one exception: its nav band left the document (`LibraryV3NavBand.cs` is fixed chrome, not a section), so it has no section for the rail to draw from and supplies `RailHead` instead (`Modes/LibraryV3Sidebar.BuildRailHead`) — **the shortcut band only**; Liked Songs' rail tile draws through the `v3.system` document section's own `ShowInRail`, and Albums/Artists/Podcasts never had one. `Pane/SidebarPaneRail.Build` prepends `Config.RailHead?.Invoke()` ahead of the plan's own tiles (a divider follows), and — because those tiles are no longer part of any plan — `SidebarPane`'s rail memo folds `Prefs.LayoutVersion.Value` into its own `DepKey` whenever `Config.RailHead` is non-null, so a TopBar edit still repaints the rail even though it moves no section. |
