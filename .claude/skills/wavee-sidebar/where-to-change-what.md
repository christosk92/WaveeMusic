...
| the V3 title flyout (the four destinations, counts, pin toggles) | `Modes/LibraryV3/LibraryV3DestinationsFlyout.cs`, opened by `LibraryV3Header.ToggleDestinations`; the destination list is `SidebarShortcutsSection.LibraryDestinations`; pinning goes through `PinActions.Pin/Unpin`. Which lens ADMITS a pinned destination (Liked Songs = a playlist; any other pinned page = the unfiltered lens only) is `SidebarEntryKinds.Admits` + `SidebarBinderPipeline.AppendUnlistedPins` (`SidebarUnlistedPinTests`). |
| the V3 header row's shape (title never truncates, when search folds, when "+" folds) | `Modes/LibraryV3/LibraryV3HeaderRules.cs` — the ONE rule for the header row: `Resolve(paneWidth, searchOpen, hasText)` decides `InlineSearch` (≥320, the field sits inline and grows), `SearchTakesRow` (<320 while open/has-text, the field covers the title) and `ShowsCreate` (≥240, else "+" folds into the "…" menu). `Modes/LibraryV3/LibraryV3Header.cs` reads it; the title itself is a button (`OnClick = _session.Collapse`, tooltip "Collapse") and is `Shrink = 0` — it is never the thing that yields. |
| the V3 search FIELD's own behaviour (Escape ladder, blur-close, the 32↔row morph) | `Modes/LibraryV3/LibraryV3SearchRules.cs` — `OnEscape` (clear-then-close), `ClosesOnBlur`; `InlineWidth` now delegates to `LibraryV3HeaderRules.InlineSearchWidth` (320) rather than owning its own threshold. The component: `Modes/LibraryV3/LibraryV3Search.cs` (`HostMorph` for the 32↔row morph; `Shrink = 0` in every shape so it never squeezes the title). |
| the V3 lens row (the count, the Albums/Artists/Podcasts page link, Sort/View) | The pure rule: `Modes/LibraryV3/LibraryV3LensRules.cs` — `Resolve(filter, qualifier, searching, drilled, paneWidth)` decides visibility (hidden while drilled — the breadcrumb takes its place), the label, the page route (Albums/Artists/Podcasts only) and whether the view glyph shows (`ViewToggleWidth` 200; below it only the sort text survives). The component: `Modes/LibraryV3/LibraryV3LensRow.cs`, mounted by `LibraryV3Chrome` right after the chips at `LibraryV3Metrics.LensRowHeight` (32). The row count comes from `LibraryV3Session.VisibleRowCount()` — the ONE place that expression is computed, so the chrome and the lens row cannot disagree. Sort/View open `V3SortViewMenu.SortRows`/`ViewRows` — Sort and View no longer live in the header's "…" menu. |
| the row anatomy / one trailing slot / pin mark / "Kind · detail" subtitle grammar (V3 only) | Geometry rules (engine-free): `Data/SidebarRowGeometry.cs` — `TrailingSlotWidth` (28), `TrailingLaneWidth` (42), `TrailingAtRest` (chevron > equalizer > empty), `HoverShowsOverflow`, `PinMarkPlacement` (`BeforeSubtitle` when the row has one, else `AfterTitle`). Subtitle grammar (engine-free): `Data/SidebarSubtitleRules.cs` — `For(entry)`/`ForRoute(routeKey, likedSongs)` map a library entry or a system route to a `SidebarSubtitleShape`, formatted through `Strings.Sidebar.V3.Kind.*`. The renderer: `Shared/SidebarEntityRow.cs` (the Slot text column + `TrailingSlot`) and `Pane/SidebarPaneSlot.cs` (`SubtitleOf(in entry, style)`/`RouteSubtitle`). All of it is gated by `Pane/SidebarPaneConfig.RowStyle` — Classic/Curated stay on the `Cluster` default and are untouched. |
| the V3 system row (Liked Songs at the top of the pinned band) | `Modes/LibraryV3/LibraryV3Document.cs` — the `"v3.system"` `StaticLinks` section, emitted before the pinned band when `state.LikedVisible` and not already covered by `SidebarShortcutsSection.ContainsRoute(topBar, "liked")`; `ShowInRail: true`, Cozy density, Artwork + Subtitles. Its subtitle ("Playlist · N songs") comes from `SidebarSubtitleRules.ForRoute` above. |
...
| the V3 nav band (Home above the header, never in the list) | `Modes/LibraryV3/LibraryV3NavBand.cs` — fixed CHROME, the first child `LibraryV3Chrome` mounts; reads `prefs.TopBar` directly and is NOT a document section for V3 (contrast Classic/Curated, which still `SidebarShortcutsSection.Prepend` it — see `LibraryV3Document.cs`'s Liked-dedupe obligation). **It carries no destinations any more** — the five-tile word rail (Playlists/Albums/Artists/Podcasts/Local files) is deleted outright (`DestinationRail`, `RailChevrons`, `RailChevron`, `ScrollRailBy` and friends are gone); the band is exactly the shortcut band's own rows. Row height: `LibraryV3Metrics.NavRowHeight`, now **32** (Compact, was 40/Cozy). Its copy of the same tiles for the 56-DIP rail is `Pane/SidebarPaneConfig.RailHead` (supplied by `Modes/LibraryV3Sidebar.BuildRailHead`, drawn first by `Pane/SidebarPaneRail.Build`) — it too draws only the shortcut band; Liked Songs' own rail tile comes from the `v3.system` document section's `ShowInRail`, not from `RailHead`. |
...
/ `SidebarRailPlannerTests` (row + tile sequences), `LibraryV3ChipStripTests` / `LibraryV3SearchRulesTests` (the
Escape/blur rules and the 320 inline-search threshold; `OpenWidth` and its tests are gone),
`LibraryV3HeaderRulesTests` (the Priority+ ladder), `LibraryV3LensRulesTests` (visibility/label/route/view-toggle),
`SidebarSubtitleRulesTests` (the "Kind · detail" grammar and the system-row subtitle), `SidebarRowGeometryTests`
(the leading-lane ladder — `ArtX(0) == 21`, `TreeContentX` = 13/25/37/49/61, `IndentFor` = 4 + 12/level — plus the
trailing-slot/pin-mark rules), `SidebarSortTests` (Recents = played, not visited).
...
| a shape change an old build cannot read | Bump `SidebarLayoutStore.CurrentVersion` and add the arm to `SidebarLayoutMigrations.Upgrade`. v1→v2 is the identity-migration precedent; `CurrentVersion = 3` now, and `MigrateV2ToV3` is the prune precedent — it drops every `Route` item whose key is `"local"` from every section (children included) and from `topBar`, in place, because Local files is retired from navigation entirely. |
