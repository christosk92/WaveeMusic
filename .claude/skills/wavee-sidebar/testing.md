...
| `SidebarShortcutsSectionTests.cs` | `SidebarShortcutsSectionTests` | 16 | **Phase 1 / Decision A.** `SidebarShortcutsSection.From`/`Renders`/`Prepend`/`ContainsRoute`; the materialisation into Classic's and Library V3's documents (incl. V3 dropping its own `v3.system` Liked row exactly when the band carries a `liked` Route item — the dedupe obligation, renamed from `v3.liked` in V3.1); and `SidebarItemCommands`' sentinel routing — a move inside Shortcuts emits `MoveTopBarItem`, a raw `MoveItem` there is an `UnknownSection` rejection. |
...
| `SidebarLayoutV2MigrationTests.cs` | `SidebarLayoutV2MigrationTests` | 11 | The version ladder through the real store; v1→v2 identity (sections, pins, V3 overlay, unknown members preserved); v2→v3 the prune — every `Route` item keyed `"local"` dropped from every section (children included) and from `topBar`, extension-data carry preserved. |
...
| `SidebarRowGeometryTests.cs` | `SidebarRowGeometryTests` | 16+ | The engine-free row-geometry ladder (`SidebarRowGeometry`): Classic⇄Curated shortcut-row height parity; the pure plan geometry (`ContentYOf`, `IndexOfRoute`, `DirectionOf`); W7's ONE leading lane — `ArtX(0) == 21` (`ContentLane` 12 + `LeadingLaneWidth` 9), `TreeContentX` = 13/25/37/49/61 (clamped at depth 4), `IndentFor` = 4 + 12/level (clamped at depth 4), and that a bare-glyph row and an art row at the same density share one label x (no more `bareGlyph ⇒ gap 12` special case); and V3.1's one-trailing-slot/pin-mark rules — the slot is 28 DIP inside a 42-DIP lane, `TrailingAtRest` (chevron > equalizer > empty), `HoverShowsOverflow` (never on a folder), `PinMarkPlacement` (leads the subtitle when one is visible, else follows the title), the pin mark's 10-DIP size and its 3/4-DIP gaps. |
...
| `LibraryV3DocumentTests.cs` | `LibraryV3DocumentTests` + `LibraryV3ViewTests` | 23 + 15 | V3 as a synthesized ephemeral document (view state → sections + query + display) and the content **order** (tree re-grouping, drill slice, materialized custom order). No shortcut section is ever prepended (W3 — the nav band left the document); the `v3.system` Liked row's dedupe against a `topBar` carrying its own `liked` route item still holds, now placed BEFORE the pinned band and followed by a `v3.rule.system` divider only when the pinned band is absent. The four destinations (Playlists/Albums/Artists/Podcasts word-rail rows) never existed as document sections and stay that way — V3.1 additionally retires Local files from the mode entirely. |
| `LibraryV3ChipStripTests.cs` | `LibraryV3ChipStripTests` | 13 | The pure filter-rail slot model (`LibraryV3ChipStrip.Slots`/`FocusIndex`, W2): idle = four unselected facets, no ✕; filtered = Clear + the one selected facet (+ its three qualifier options, Playlists only, only when the data evidences them); fused = Clear + one `Fused` slot sharing the loose facet's key (the morph); what each slot kind writes back on a tap; `FocusIndex` surviving a loose⇄fused relayout because the key is shared. Facet order is Playlists · Albums · Artists · Podcasts. |
| `LibraryV3HeaderRulesTests.cs` | `LibraryV3HeaderRulesTests` | new | The Priority+ ladder (`LibraryV3HeaderRules.Resolve`): `Resolve_*` theories for 180/200/239/240/319/320/460 × `searchOpen` — `InlineSearch` at ≥320, `SearchTakesRow` while narrow and open/has-text, `ShowsCreate` at ≥240. The title never yields; it is `Shrink = 0` in every shape. |
| `LibraryV3LensRulesTests.cs` | `LibraryV3LensRulesTests` | new | `LibraryV3LensRules.Resolve`: hidden while drilled, the label per filter/qualifier, the page route (Albums/Artists/Podcasts only — never for Playlists/All), and `ShowsView` at the `ViewToggleWidth` (200) threshold. |
| `LibraryV3SearchRulesTests.cs` | `LibraryV3SearchRulesTests` | 10 | The pure search-FIELD model (`LibraryV3SearchRules`, W1): the Escape ladder (clear-then-close) and blur-closes-only-when-empty. `InlineWidth` now delegates to `LibraryV3HeaderRules.InlineSearchWidth` (320); `OpenWidth`, `TitleReserve`, `TrailingControlsWidth` and their `OpenWidth_*` theories are **gone** — the open narrow field takes the row (`Grow = 1`) and the title hides via `Flow`, so no width arithmetic is left to test here. |
| `SidebarSubtitleRulesTests.cs` | `SidebarSubtitleRulesTests` | new | `SidebarSubtitleRules.For`/`ForRoute`, engine-free: per-kind subtitle shape (playlist song-count vs. owner — `ShowsOwner`'s exact conditions, including `IsOwner` beating a stale flavor; album first-billed-artist-then-creator-then-bare; artist bare; show publisher-or-bare; folder item count; track creator-or-empty) and the system row (`ForRoute("liked", n)` → songs, `("liked", null)` → bare Playlist, any other/unknown key → empty). |
...
| `SidebarPinStoreTests.cs` | `SidebarPinStoreTests` | 19+ | The shared pin store + `SidebarPinId` mapping (rides `VirtualCollectionSignalShim` for `Signal<int>`). V3.1: a persisted route pin whose key `SidebarPinId.IsPinnableRoute` no longer accepts (a retired `"local"` pin) is dropped silently on load rather than surfaced as a dead pin. |
...
**V3.1 additions:** the header title stays intact (never ellipsizes) at pane widths 180 / 240 / 320 / 460, with and
without search open · no word rail anywhere above the header · the lens row's states — All / a facet / a facet
with qualifier / searching (match count) / drilled (breadcrumb replaces it) / empty-by-filter — including the
Albums/Artists/Podcasts link and the Sort/View controls · the Liked Songs system row at the top of the pinned band
with its dynamic cover and "Playlist · N songs" · every V3 row showing exactly one trailing slot (empty at rest,
"…" on hover, the equalizer on the playing row, a chevron on folders) with the pin mark leading a pinned row's
subtitle · Classic and Curated rows unchanged side by side with V3 · **no "Local files" anywhere** — not in
Classic, not in the customizer's Destinations palette, not in the pin picker, and a `local` deep link refused.
