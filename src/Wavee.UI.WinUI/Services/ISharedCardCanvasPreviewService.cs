...
public interface ISharedCardCanvasPreviewService
{
    ...
    /// <summary>Pre-open (warm) a canvas's MediaSource so a subsequent <see cref="AcquireAsync"/> for the
    /// same URL starts playback near-instantly. Best-effort; never throws.</summary>
    Task PreloadAsync(string canvasUrl, CancellationToken ct = default);
}
