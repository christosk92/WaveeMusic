...
public sealed class FakePodcastSource : IPodcastSource
{
    ...
    public Task<Episode?> GetEpisodeAsync(string uri, CancellationToken ct = default)
    {
        var parts = uri.Split(':');
        if (parts.Length != 4 || !int.TryParse(parts[2], out var showIndex)) return Task.FromResult<Episode?>(null);
        var showUri = "wavee:show:" + showIndex;
        return Task.FromResult(FakeData.Show(showUri)?.Episodes?.FirstOrDefault(episode => episode.Uri == uri) is { } episode
            ? episode with { ShowUri = showUri } : null);
    }
    ...
    public Task<Show?> GetShowAsync(string uri, CancellationToken ct = default)
