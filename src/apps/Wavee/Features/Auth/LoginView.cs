...
//
// What survives is what the wizard actually composes: the OpenUrl hop and the four live sub-components below
// (LoginStepRow/LoginStepBar for the Finalizing ladder, WaitingDots, LoginCountdown). The Spotify brand tint
// (SpotifyGreen) is gone with the wizard's green primary — the setup footer is the stock WinUI accent button now. The rest of the old takeover's chrome
// (CompactRightPane/CompactPairingLink, OrDivider, BrowserLoginButton, GlyphBadge, SpotifyBrand, GoldTint) is DELETED
// outright (CLAUDE.md: no legacy paths) — the Rise 1:1 visual rework's SignIn page composes its own SettingsCards and
// mounts QrGrid directly instead (Features/Setup/SetupPage.SignIn.cs: SetupScanCard).
//
// The name stays `LoginView` deliberately: it is referenced as `LoginView.OpenUrl` from
