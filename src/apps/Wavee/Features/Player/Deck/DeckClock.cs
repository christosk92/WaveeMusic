...
{
    /// <summary>30 Hz — the foreground ambient cadence. Motion IS presents here (there is no partial repaint), so
    /// this rate is the deck's present rate; the value-gated writes below are what keep most of those elided.</summary>
    public const float TickMs = 1000f / 30f;
    ...
    public override Element Render()
    {
        ...
        bool pwr = b.PlayWhenReady.Value;
        bool buffering = b.IsBuffering.Value;
        bool err = b.Error.Value is not null;
        // Through a memo: the transport republishes several times a second (its position upper bound moves), and the
        // only part of it this component's STRUCTURE cares about is the phase.
        var phase = UseComputed(() => Bridge.Transport.Value.Phase).Value;
        bool ended = phase == PlaybackPhase.Ended;
                                 ...
                                 DeckBoundary boundary, long? syntheticSeek)
    {
        ...
        bool advancing = transport.OutputAdvancing && !buffering;
        long dur = b.DurationMs.Peek();
        long? seek = b.SeekTargetMs.Peek() ?? syntheticSeek;
        long position = pos.Estimate(nowMs, advancing, seek, transport.PositionUpperBoundMs, dur);
        return new DeckInput(
            nowMs,
            b.CurrentTrack.Peek() is not null,
            transport.Phase,
            transport.PlayWhenReady,
            advancing,
            buffering || (transport.PlayWhenReady && transport.Phase is PlaybackPhase.Resolving or PlaybackPhase.Buffering),
            b.Error.Peek() is not null,
            transport.Phase == PlaybackPhase.Ended && !transport.PlayWhenReady,
            boundary,
            seek,
