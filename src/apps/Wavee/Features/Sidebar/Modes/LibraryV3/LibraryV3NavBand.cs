...
sealed class LibraryV3NavBand : Component
{
    ...
    //    no drag source, no drop target, no context menu, no reorder, no multi-select). COMPACT (32-DIP), not Cozy:
    //    this band used to be a 40-DIP Cozy row with a full-width selected plate drawn ABOVE the library's own title,
    //    which inverted the hierarchy — a shortcut read as more important than the section it sits on top of. Compact
    //    with the standard gutter (SidebarRowMetrics.ArtFor(SidebarDensity.Compact)) fixes that: the band now reads as
    //    quiet chrome, and the five fixed library destinations it once carried are gone — Liked Songs is a system row
    //    owned by the document (W5), Albums/Artists/Podcasts are the lens row's page link (W2), and Local files is
    //    retired from every navigation surface (W7). ─────────────────────────────────────────────────────────────────
    ...
    Element ActionRow(SidebarItemSpec item, WaveeExtensionRegistry? registry)
    {
        ...
        var spec = new SidebarRowSpec
        {
            ...
            Density = SidebarDensity.Compact,
            Height = LibraryV3Metrics.NavRowHeight,
            // Art-wide leading column + the row's own LeadingGap (W7): the action row's label lines up with the
            // route rows beside it, at the same Compact gutter (W3).
            Leading = SidebarPaneIcon.Leading(item.IconOverride, icon, enabled, SidebarRowMetrics.ArtFor(SidebarDensity.Compact)),
            OnClick = click,
            Focusable = enabled,
        };
    ...
    Element TrackRow(SidebarItemSpec item)
    {
        ...
        var spec = new SidebarRowSpec
        {
            ...
            Density = SidebarDensity.Compact,
            Height = LibraryV3Metrics.NavRowHeight,
            Leading = SidebarCover.Art(SidebarPaneText.FallbackImage(item), null, uri, SidebarRowMetrics.ArtFor(SidebarDensity.Compact)),
            Track = true,
