// ── Screens/ReleaseNotes.Model.cs — CORE (owner R, wave 6; plan §2 · the file's own budget is 850) ─────────────────
//
// THE DOCUMENT + INDEX RECORDS. A DERIVED split of ch 28 §9.5's 1,850-line `ReleaseNotes.cs` budget: this half is
// the named partial ch 28 §9.3.6 asks for (plan §2's "Two notes on Screens/"), so `Wavee.ReleaseTool`'s
// `<Compile Include>` cherry-pick list has a name to include (plan §3.3). The other half — the parsers, links and
// validation — is `Screens/ReleaseNotes.cs`.
//
// Spec: ch 28 §9.5 / §9.3.6.
//
// THESE TWO FILES (this one and `ReleaseNotes.cs`) ARE THE EXACT CHERRY-PICKS `Wavee.ReleaseTool.csproj` compiles
// (plan §3.3) — a third partial added to this concern is a build break in that project, not a behavior change here.
//
// `AppUpdateToasts` is NOT in this file (A8) — it belongs to `Platform/Notify.cs`.
//
// Ported verbatim from `src/apps/_old/Wavee.Core/ReleaseNotes/ReleaseNotesDocument.cs` and
// `.../ReleaseNotesJsonContext.cs` (namespace `Wavee.Core.ReleaseNotes`, now nested under `Wavee.ReleaseNotes`);
// only the namespace, the nesting and the usings moved — bodies, comments and doc comments are unchanged.
