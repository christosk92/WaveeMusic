...
static class WaveeSettings
{
    ...
    // Lyrics depth-of-field + glow-halo blur STRENGTH, 0..100%. -1 (the default) is AUTO: LyricsBlurPolicy.Resolve
    // picks 100 on a normal GPU and 40 on GpuProfile's weak tier, so a fresh install on an iGPU is not silently
    // handed the full σ ladder's idle GPU cost. 0 turns the effect off entirely (LyricsBlurPolicy.Enabled). An
    // explicit slider move always overrides the auto pick, on any tier, until the user resets it.
    public static readonly SettingKey<int> LyricsBlurStrength = new("appearance.lyrics.blurStrength", -1);
