// The lifecycle state machine through an injectable transport: lazy start, the handshake, a crash and its restart, the
// Faulted latch after three failed starts, Retry, idle stop, and the stream lease that blocks it — ported from 0.2.9
// Wavee.Tests/Modules/ModuleProcessLifecycleTests.cs onto `Modules.ModuleProcess`. No real process is ever spawned.
...
public class ModuleProcessLifecycleTests
{
    ...
    sealed class Harness : IModuleHostSink, IAsyncDisposable
    {
        ...
        public Harness(FakeModule script, InstalledModule? module = null)
        {
            ...
            Process = new ModuleProcess(Module, this,
                (_, _, _) =>
                {
        ...
        public void OnMetadata(string moduleId, MetadataUpdate update) { lock (Metadata) Metadata.Add(update); }
        public void OnExpired(string moduleId, string playableId) { lock (Expired) Expired.Add(playableId); }
    ...
    static FakeModule Resolvable() => new() { Resolve = p => ModuleFixtures.Resolved(p.PlayableId) };

    static Task<ResolvedPlayable> Resolve(ModuleProcess p, string id = "p1")
        => p.RequestAsync(ModuleMethods.Resolve, new ResolveParams(id, null), SdkJsonContext.Default.ResolveParams,
            SdkJsonContext.Default.ResolvedPlayable, ModuleTimeouts.Resolve, Ct);
    ...
    public async Task Handshake_PublishesTheEffectiveCapabilities()
    {
        ...
        Assert.False(h.Process.Declares("match"));   // the EFFECTIVE list narrows the manifest's
    }

    [Fact]
    public async Task TheHandshakeCarriesTheHostsRangeLocaleAndPrefs()
    {
        InitializeParams? seen = null;
        var script = Resolvable();
        script.Initialize = p => { seen = p; return new InitializeResult(1, ["playback"], null); };
        await using var h = new Harness(script);

        await Resolve(h.Process);

        Assert.NotNull(seen);
        Assert.Equal("1.0.0-test", seen.HostVersion);
        Assert.Equal(ModuleCatalog.MinProtocol, seen.MinProtocol);
        Assert.Equal(ModuleCatalog.MaxProtocol, seen.MaxProtocol);
        Assert.Equal("en-US", seen.Locale);
        Assert.Equal("", seen.DataDir);   // no data root configured: never a real %LOCALAPPDATA% path in a test
    }
    ...
    public async Task ACrashWhileReady_MovesToCrashed_AndTheNextRequestRestarts()
    {
        ...
        h.Last.Crash();
        await WaitAsync(() => h.Process.State == ModuleProcessState.Crashed);

        h.Advance(TimeSpan.FromSeconds(5));   // the backoff is clock-driven
        await Resolve(h.Process);
    ...
    public async Task ModuleNotifications_ReachTheHostSink()
    {
        ...
        h.Last.Module.Notify(ModuleMethods.Metadata, new MetadataUpdate("p1", "Now playing", ["Someone"], null), SdkJsonContext.Default.MetadataUpdate);
        h.Last.Module.Notify(ModuleMethods.Expired, new ExpiredNotification("p1"), SdkJsonContext.Default.ExpiredNotification);

        await WaitAsync(() => { lock (h.Metadata) lock (h.Expired) return h.Metadata.Count > 0 && h.Expired.Count > 0; });
    ...
    public async Task AModuleErrorIsRaisedTyped_AndCounted()
    {
        ...
        {
            Resolve = _ => throw new ModuleException(ModuleErrorCode.GeoBlocked, "not in your country") { RetryAfterMs = 5_000, Detail = "geo" },
        };
        await using var h = new Harness(script);
    ...
    public async Task AnUnimplementedMethod_ReadsAsCapabilityAbsent_NotAFailureOfTheModule()
    {
        ...
        var ex = await Assert.ThrowsAsync<ModuleException>(() => h.Process.RequestAsync(ModuleMethods.Action, new ModuleActionParams("x"),
            SdkJsonContext.Default.ModuleActionParams, SdkJsonContext.Default.ModuleActionResult, ModuleTimeouts.Diagnostics, Ct));
    ...
    [Fact]
    public void Stats_ReportsPercentilesOverTheRing()
    {
        var stats = new ModuleStats();
        for (int i = 1; i <= 100; i++) stats.NoteRequest(i);
        stats.NoteFailure("playback/resolve", "boom");
        stats.NoteRestart();

        ModuleStatsSnapshot s = stats.Snapshot();
        Assert.Equal(100, s.Requests);
        Assert.Equal(1, s.Failures);
        Assert.Equal(1, s.Restarts);
        Assert.Equal(50, s.P50Ms);
        Assert.Equal(95, s.P95Ms);
        Assert.Equal("boom", s.LastError);
        Assert.Equal("playback/resolve", s.LastErrorMethod);
    }
