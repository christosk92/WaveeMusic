// The WATCH page's projection (`Modules.WatchPageModel`) — every layout DECISION a YouTube-shaped module page makes,
// asserted as VALUES — ported from 0.2.9 Wavee.Tests/Modules/WatchPageModelTests.cs. The projection is pure (System +
// Wavee.Sdk), so these are behavioural assertions rather than a scan of the view's source text. The staged-identity
// fact is re-stated against the 0.3 arbitration (`Video.DockedHosting`), the one the page's claim feeds (G-151).
...
public class WatchPageModelTests
{
    ...
    public void TheStagedIdentityIsThePlayable_NotThePageEntity()
    {
        ...
        Assert.True(DockedHosting.PageStageHosts(staged, playing));
        Assert.False(DockedHosting.PageStageHosts(pageUri, playing));   // the regression, pinned

        Assert.True(DockedHosting.ShouldMount(DockedFace.PageStage, SurfacePlacement.Docked,
            staged, staged, playing));
        Assert.False(DockedHosting.ShouldMount(DockedFace.PageStage, SurfacePlacement.Docked,
        ...
        Assert.Equal(DockedHost.PageStage,
            DockedHosting.HostFor(SurfacePlacement.Docked, staged, playing));
        Assert.Equal(DockedHost.Rail,
            DockedHosting.HostFor(SurfacePlacement.Docked, pageUri, playing));
    }
