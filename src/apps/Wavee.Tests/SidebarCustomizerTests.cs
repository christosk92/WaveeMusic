...
// The companion page's PURE model (Phase 3): the searchable palette table + filter — including the DESTINATIONS
// group — the display-option projection the generated property controls bind, and the opaque extension-config
// rewriter. All of it lives in Shell/Sidebar.cs (+ Sidebar.Doc.cs for the reducer/document types it composes
// against), which is engine-free ON PURPOSE and exercised here directly, so these tests drive PRODUCTION code
...
// (`SidebarOutlineRow`/`SidebarOutlineRows`/`SidebarOutlineDrag`) all died WITH the surfaces they described — the
// page is ONE scrolling column at every width and the docked pane IS the canvas. The one section-drag translation
// that survives is `SidebarEditPlan.ToMoveSection`, covered by SidebarEditPlanTests (not ported here).
//
// 0.3 CHANGES ported from 0.2.9 (plan §9.6 Q7, 2026-09-12), verified against Sidebar.cs before writing:
//   1. `api-console` is DELETED as a destination — it is not in the palette at all.
//   2. `ExtraDestinationRoutes` is 2 entries (settings, concerts), not 3 — api-console is gone.
//   3. There is no developer-mode route gate: 11 destination rows always, for every user. `DeveloperMode` is gone.
public class SidebarCustomizerTests
{
    // ── the palette table ─────────────────────────────────────────────────────────────────────────────────────────────
    ...
    ///
    /// <para>A SECTION row spells its own <c>NameLocKey</c>. A DESTINATION row deliberately leaves it EMPTY and
    /// carries a <c>RouteKey</c> instead, because "what this page is called" already has a single owner (the route
    /// table at the UI edge — the tab strip, the breadcrumb, a pinned row). Minting a second spelling here is
    /// exactly the drift the single-owner rule exists to catch. So the rule is EITHER a non-empty name key OR a
    /// non-empty route key, never neither.</para></summary>
    [Fact]
    ...
    /// <summary>0.3 CHANGE: `api-console` no longer exists as a destination, and there is no developer-mode gate —
    /// every one of the 11 destination rows is offered to every caller of <c>Filter</c>. "Everything" is really
    /// everything now, unlike 0.2.9 where Filter dropped the (still-present) api-console row while developer mode
    /// was off.</summary>
    [Fact]
    public void Palette_FilterIsTokenWiseAndCaseInsensitive()
    {
        ...
        Assert.Equal(SidebarPalette.All.Length, SidebarPalette.Filter("", Label, Description, into));
    ...
    //
    // The fix for "search home, get Links": typing "home" used to return one row — *Links — Shortcuts to pages like
    // Home or Search* — whose click added an EMPTY StaticLinks section. Destinations answer the question with the
    // page.
    ...
    /// <summary>0.3 CHANGE: the entry SET is still derived, never re-typed — <c>SidebarPinId.PinnableRoutes</c> ∪ the
    /// TWO real pages that list omits, now that api-console is gone entirely: settings (the pin policy refuses it as
    /// a tooling surface) and the concerts hub (the curated pin picker does not advertise it, though it is pinnable
    /// once reached). 11 rows always, for every user — there is no developer-mode gate on this table.</summary>
    [Fact]
    public void Destinations_AreThePinnableRoutesPlusTheTwoUnpinnablePages()
    {
        var routes = new List<string>();
        foreach (var e in SidebarPalette.Destinations) routes.Add(e.RouteKey!);

        Assert.Equal(SidebarPinId.PinnableRoutes.Length + 2, routes.Count);
        ...
        Assert.Equal(new[] { "settings", "concerts" },
                     routes.GetRange(SidebarPinId.PinnableRoutes.Length, 2).ToArray());

        // The extras are exactly the pages PinnableRoutes omits — otherwise this list would duplicate one row.
        foreach (var extra in new[] { "settings", "concerts" })
        ...
        // the tooling surface is refused by the pin scheme outright, the concerts hub is pinnable but uncurated.
        ...
        // api-console is not a destination at all — deleted outright, not merely gated.
        Assert.DoesNotContain("api-console", routes);

        // "home" is in there, which is the whole point of the group.
        Assert.Contains("home", routes);
    }

    /// <summary>Every destination is ONE undoable <c>AddSection(StaticLinks, Item: route)</c> — a pre-seeded section,
    /// so the click produces a WORKING row rather than defect 7's empty grey hint. Its id is namespaced <c>dest:</c>
    /// so it can never collide with a section entry's id.</summary>
    [Fact]
    ...
    /// <summary>A destination clicked while a <c>StaticLinks</c> section is the subject APPENDS to it instead of
    /// minting a sibling — eleven destinations would otherwise be eleven one-row sections. Only a destination does
    /// this, and only into StaticLinks: appending a route into a PlaylistTree would be a
    ...
    /// <summary>CanDrag is about whether ONE <c>AddSection</c> can be composed at drag promotion. Three shapes
    /// cannot: the two that open a modal first (an action shortcut, a bare Links section — the picker IS the
    /// gesture, and a dialog opening mid-drag would be absurd), the one that switches the palette into
    /// contribution-pick mode, and "Recently played", which is deliberately TWO commands. Those stay CLICK-ONLY
    /// rather than shipping a drag that lies about its outcome.</summary>
    [Fact]
    ...
    /// <summary>DEFECT 7 — the bare "Links" row no longer adds a zero-item section. It opens the destination picker
    /// on the way in, which is a different <c>Add</c> verb, which is also why it is click-only above.</summary>
    [Fact]
    ...
    /// <summary>The static field-initializer order the table's own doc comment demands: C# runs them in TEXTUAL
    /// order, so <c>ExtraDestinationRoutes</c> must be declared ABOVE <c>Destinations</c> or <c>BuildDestinations</c>
    /// would see it null and ship an empty Destinations group. Read directly against the source rather than
    /// inferred from behaviour, since a working <c>Destinations</c> array does not by itself prove the ordering is
    /// the SOURCE of that value rather than a coincidence of static-init defaults.</summary>
    [Fact]
    public void Destinations_ArrayIsNonEmptyWhichOnlyHoldsIfExtraRoutesInitializedFirst()
    {
        // BuildDestinations() reads ExtraDestinationRoutes; if that field were declared below Destinations, C#'s
        // textual static-init order would leave it null at that point and BuildDestinations would either throw or
        // (if guarded) silently ship 9 rows instead of 11. This is the externally observable half of that contract.
        Assert.Equal(11, SidebarPalette.Destinations.Length);
    }
    ...
    /// <para>This is the guard the round-2 "some things don't change" report demanded. A silent OFF-BY-ONE between
    /// the panel's choice index and the reducer's enum cast is invisible in the UI — the control moves, the document
    /// changes, and the WRONG value lands — so it can only be caught here. The enum arms are the prime suspects (a
    ...
    /// <summary>Exactly the values the property panel can send for one field: 0/1 for a flag, every choice index for
    /// an enum, and the range endpoints plus an interior point for a number. Nothing outside this set is reachable
    /// from the UI, and everything inside it must survive the trip.</summary>
    ...
    /// <summary>Every enum field's choice-label list must be exactly as long as the value range the reducer accepts.
    /// A list SHORTER than the range hides a legal value from the user; a list LONGER than it offers a choice the
    /// reducer silently clamps away — which is the "I picked it and nothing happened" shape of the round-2
    /// report.</summary>
    [Fact]
    public void DisplayValues_EnumChoiceListsMatchTheReducersAcceptedRange()
    {
        foreach (var field in SidebarDisplayValues.Order)
        {
            ...
            // One past the last offered choice must CLAMP back onto the last one. If it does not, the reducer
            // accepts a value the panel never offers and the choice list is short.
    ...
    public void ConfigWrite_RoundTripsThroughTheReducersSetExtensionConfig()
    {
        ...
        // Re-applying the same value is a NoChange (the reducer compares canonical JSON) — so a mirror effect that
        // writes the control's own value back can never burn an undo slot.
