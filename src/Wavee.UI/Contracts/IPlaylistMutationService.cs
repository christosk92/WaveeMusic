...
public interface IPlaylistMutationService
{
    ...
    /// <summary>
    /// POSTs a chip-click signal to the playlist's session-control endpoint and
    /// applies the re-personalised <c>SelectedListContent</c> the server hands
    /// back to the playlist cache. Returns true on success, false when the
    /// session is offline / the signal request fails.
    /// </summary>
    /// <param name="playlistUri">Full <c>spotify:playlist:{id}</c> URI.</param>
    /// <param name="revision">Current playlist revision bytes from the cache.</param>
    /// <param name="signalKey">Server-advertised identifier of the form
    /// <c>session_control_display$&lt;group_id&gt;$&lt;option_key&gt;</c>.</param>
    /// <param name="requestId">Fresh GUID for correlation.</param>
    Task<bool> SendPlaylistSignalAsync(
        string playlistUri,
        byte[] revision,
        string signalKey,
        string requestId,
