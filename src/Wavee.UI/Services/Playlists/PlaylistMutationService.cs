...
public sealed class PlaylistMutationService : IPlaylistMutationService
{
    ...
    public async Task<bool> SendPlaylistSignalAsync(
        string playlistUri,
        byte[] revision,
        string signalKey,
        string requestId,
        CancellationToken ct = default)
    {
        ...
        try
        {
            var fresh = await _session.SpClient
                .SendPlaylistSignalAsync(playlistUri, revision, signalKey, requestId, ct)
                .ConfigureAwait(false);
            ct.ThrowIfCancellationRequested();

            await _playlistCache.ApplyFreshContentAsync(playlistUri, fresh, ct).ConfigureAwait(false);
            return true;
        }
