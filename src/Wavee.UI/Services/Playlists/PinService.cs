    ...
    public async Task<IReadOnlyList<PinnedItemDto>> GetPinnedItemsAsync(CancellationToken ct = default)
    {
        ...
        foreach (var e in entities)
        {
                    ...
                    && e.Uri.EndsWith(":collection", StringComparison.Ordinal)))
            {
                ...
                {
                    Uri = e.Uri,
                    // Title stays the well-known literal; the sidebar binding
                    // localizes it via its own resource lookup. Kept literal so
                    // this service can move down to Wavee.UI without dragging
                    // in the WinUI-only ResourceLoader.
                    Title = "Liked Songs",
                    ImageUrl = null,
